...
export class ProdutoNoPedidoComponent implements OnInit {
  ...
  protected onSuccess(data: IProdutoNoPedido[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/produto-no-pedido'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
